package com.skittle.backrooms.misc.itemblock;
...
public class ItemBlockDoor extends ItemBlock
{
	...
	private static void placeDoor(World worldIn, BlockPos bottomDoorPos, EnumFacing playerFacing, Block door, boolean isRightHinde)
	{
		...
		if ((!flag || flag1) && j <= i)
		{
			if (flag1 && !flag || j < i) isRightHinde = false;
		}
		else isRightHinde = true;
		...
		IBlockState doorState = door.getDefaultState()
				...
				.withProperty(BlockDoor.HINGE, isRightHinde ? BlockDoor.EnumHingePosition.RIGHT : BlockDoor.EnumHingePosition.LEFT)
